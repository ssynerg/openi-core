...
impl PanicLoopReflex {
    ...
    fn extract_flag(&self, evt: &Envelope) -> bool {
        // If JSON pointer fails, default to false.
        let find = |v: &serde_json::Value| -> Option<&serde_json::Value> {
            let mut cur = v;
            for seg in self.field_pointer.trim_start_matches('/').split('/') {
                cur = cur.get(seg)?;
            }
            Some(cur)
        };

        let header_hit = find(&evt.headers).and_then(|v| v.as_bool()).unwrap_or(false);
        let body_hit = find(&evt.body).and_then(|v| v.as_bool()).unwrap_or(false);
        header_hit || body_hit
    }
}
...
// Re-export types for crate users.
pub use PanicLoopReflex;
pub use PolicyGuardReflex;
pub use RateLimitReflex;
