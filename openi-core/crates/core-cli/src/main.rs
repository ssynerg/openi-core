...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Node => run_node(),
        Cmd::Help => { println!("{}", Cli::command().render_help()); Ok(()) }
    }
}
...
fn package_manifest(path: &str) -> anyhow::Result<()> {
    use std::fs;
    let p = if path.ends_with('/') { &path[..path.len()-1] } else { path };
    let data = fs::read_to_string(format!("{}/AgentManifest.yaml", p))?;
    let doc: serde_yaml::Value = serde_yaml::from_str(&data)?;
    println!("Validated Agent Manifest: kind={}", doc.get("kind").and_then(|k| k.as_str()).unwrap_or("?"));
    Ok(())
}
